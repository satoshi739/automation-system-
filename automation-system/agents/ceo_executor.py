...
## 最低保証（必須）
**1サイクルで必ず最低5件のタスクを create_agent_task で作成すること。** キューが空のサイクルは「今日は何もしない日」ではなく「フルスロットルで毎ブランドのコンテンツを生成する日」である。
最低構成:
- 各コンテンツエージェント（upj/dsc/cfj/bpg）にそれぞれ日次SNSコンテンツ生成タスクを1件以上（合計4件）
- agent-analytics または agent-sales に分析/フォローアップタスクを1件以上
タスクを作らずに dispatch_done を呼ぶことは禁止。
"""
...
分析が完了したら dispatch_done を呼んで終了してください。"""
    ...
    for iteration in range(max_iterations):
        ...
        log.info(f"CEO iteration {iteration}: stop_reason={response.stop_reason}")

        # アシスタントターンを追加
        assistant_content = response.content
        messages.append({"role": "assistant", "content": assistant_content})

        # Claude のテキスト返答もロギング（dispatch_done を呼ばずに end_turn する原因究明用）
        for _b in assistant_content:
            if getattr(_b, "type", "") == "text":
                _txt = (getattr(_b, "text", "") or "").strip()
                if _txt:
                    log.info(f"CEO text (iter={iteration}): {_txt[:500]}")
    ...
    # 0タスク作成は異常: 翌日タスクが空になりシステムが停滞するため alert
    if tasks_created == 0:
        log.warning("CEO dispatch produced 0 tasks — system will idle until next cycle")
        try:
            from sns.line_api import LINEMessenger
            owner_id = os.environ.get("OWNER_LINE_USER_ID", "")
            alert_token  = os.environ.get("ALERT_LINE_CHANNEL_ACCESS_TOKEN", "")
            ...
            if owner_id and alert_token and alert_secret:
                msg = (
                    "⚠️ AI CEO ディスパッチが 0タスクで終了しました。\n"
                    f"summary: {final_summary or '(空)'}\n"
                    "scheduler.log で CEO text を確認してください。"
                )
                LINEMessenger(token=alert_token, secret=alert_secret).push(owner_id, msg)
        except Exception as _e:
            log.warning(f"0タスクアラート送信失敗: {_e}")
